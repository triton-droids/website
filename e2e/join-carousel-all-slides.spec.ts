...
test.describe('Join Carousel All Slides', () => {
  test('Capture all 4 slides of Join carousel with highlighting', async ({ page }) => {
    ...
    let card = page.locator('div[class*="bg-[#2A2B2D]"]').filter({
      has: page.locator('h3:has-text("Real world impact")')
    }).first();
    await card.screenshot({ path: 'e2e/screenshots/join-slide-1-real-world-impact.png' });
    ...
    card = page.locator('div[class*="bg-[#2A2B2D]"]').filter({
      has: page.locator('h3:has-text("Hands-on experience")')
    }).first();
    await card.screenshot({ path: 'e2e/screenshots/join-slide-2-hands-on.png' });
    ...
    await card.screenshot({ path: 'e2e/screenshots/join-slide-4-connections.png' });
